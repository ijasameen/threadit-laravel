...
<x-layout.app>
    <div class="w-full max-w-3xl mx-auto space-y-3">
        ...
        @endforeach
        @auth
            <form method="POST" action="{{ route('replies.update') }}" class="mx-5 mb-3 space-y-2 flex flex-col">
                @csrf
                @method('PATCH')

                <div>
                    <input name="reply_id" type="text" hidden value="{{ $reply->id }}">
                    <input name="post_id" type="text" hidden value="{{ $post->id }}">
                    <x-form.text-area name="body" id="body" autocomplete='body' placeholder="Reply"
                        :value="$reply->body" rows=3 />
                    <x-form.error :messages="$errors->get('body')" />
                </div>

                <button type="submit" class="btn btn-primary self-end w-full max-w-28">Save</button>
            </form>
        @endauth
