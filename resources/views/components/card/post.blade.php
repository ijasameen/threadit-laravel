    ...
    <div class="card-body pb-3 pt-5 px-6">
        ...
        <div class="flex gap-x-2">
            ...
            <a href="{{ $link }}" type="button"
                ...
                <span class="pointer-events-none icon-[tabler--message-circle] size-5"></span>
                <span class="pointer-events-none">{{ count($post->replies) }}</span>
            </a>
            <button type="button" {{ $user?->isSavedPost($post) ? 'data-saved="on"' : '' }} data-savable-type="post"
                data-savable-id="{{ $post->id }}"
